...
This document dives into predicting the 2023 AFL Brownlow Medal votes, using a dataset compiled from various reputable sources such as the `fitzRoy` R package, afl.com.au, AFL Tables, and the AFL Coaches Association. All of this raw data is gathered, cleaned and stitched together by a companion project of mine, [AFLPlayerStats](https://github.com/CooperDenny/AFLPlayerStats), before it ever reaches this document - that repository is what does the heavy lifting of scraping player stats, team results and coaches votes season by season.

This is the first outing for this model, so there's no prior year's version to compare it to just yet - everything here, from the feature engineering to the choice of an ordinal logistic regression, is being tried for the first time. One quirk worth flagging up front: because a handful of the AFL's tracked statistics (like kick-ins) have only been recorded from 2019 onwards, the model formula below is built conditionally, using a fuller set of predictors when the training window extends into 2019 or later.
...
Top of the list is Jesse Hogan's Round 23 performance for GWS against Essendon, edging out a number of other big individual games from around the competition.
...
Our model has done pretty well. While it didn't correctly predict the eventual winner in Lachie Neale, it wasn't far off - the model had Nick Daicos out in front on 31 predicted votes, with Marcus Bontempelli next on 30, and Neale himself third on 28. As it turned out, Neale won with 31 votes, Bontempelli was runner-up on 29, and Daicos third on 28 - practically the same three names, just shuffled by a vote or two. In terms of error metrics, the Mean Absolute Error (MAE) is `r round(mean(abs(vote_tally$Predicted.Votes - vote_tally$Actual.Votes)),3)`, and the Root Mean Square Error (RMSE) is `r round(sqrt(mean((vote_tally$Predicted.Votes - vote_tally$Actual.Votes)^2)),3)`.
