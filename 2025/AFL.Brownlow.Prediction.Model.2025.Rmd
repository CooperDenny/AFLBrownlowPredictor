---
title: "<b>Predicting the 2025 Brownlow Medal</b>"
author: "Cooper Denny"
...
In this document, I constructed an ordinal logistic regression model to predict the 2025 AFL Brownlow Medal votes. To do so, I used a dataset I compiled from various trusted sources, including the fitzRoy R package, afl.com.au, AFL Tables, and the AFL Coaches Association. That data collection and cleaning work now lives in its own repository, [AFLPlayerStats](https://github.com/CooperDenny/AFLPlayerStats), which I run each week to keep the underlying player, team and coaches vote data current - this document simply reads in the finished product.

This year's model is a straight continuation of the 2024 version, `Position` variable and all. With two years of that variable now in the training data (2024 and 2023 both included in some capacity), I was curious whether the ruckman-vote-inflation problem it was designed to fix in 2023 had stayed fixed, or crept back in with a new training season added.

For my 2025 Brownlow prediction modelling, I trained the model on home-and-away season data from 2015-2024, with the goal to forecast how many Brownlow votes a player might receive based on their performance metrics.

Once the model was trained, I shifted to the testing phase, where I evaluated its performance using player data from 2025. I conducted a game-by-game analysis to compare my predictions with the actual Brownlow votes awarded by umpires, providing insights into how well the model performed. I also took a broader look at season-wide predictions, comparing my predicted votes with the actual counts to assess the model’s accuracy and predictive capabilities.
...
To begin, I specified the time periods for training and testing the model. The training period covered data from 2015 to 2024, while the testing period focused on the 2025 season:
...
After a quick peek of the 2025 data, I then standardise the numeric values on the same scale as the training set, as well as create dummy variables for the categorical variables `Coaches.Votes`, `Team.Result` and `Position`.
...
So, according to the model, Jeremy Cameron's Round 20 performance for Geelong against North Melbourne was considered the best game according to `Expected.Votes`, followed by a number of notable individual performances from other stars of the competition.
...
This year the model's top pick didn't quite get there. Nick Daicos led the predicted leaderboard on 36 votes and ultimately finished runner-up with 32 actual votes - a solid, defensible read. But the real story of 2025 was Matt Rowell, whose 39 actual votes were enough to win the Medal outright; the model had him down in sixth on just 25 predicted votes, clearly underestimating just how much of an impact he was having each week. It's a useful reminder that a model built on standard box-score statistics can miss a player whose value shows up more in relentless pressure and contested-ball work than in eye-catching raw numbers.
...
In conclusion, this year's result is a useful counterweight to 2024's near-perfect top two. Missing Matt Rowell's Medal win entirely shows the limits of a model that's trained on the same core box-score statistics every year - it's very good at rewarding players who consistently accumulate the traditional numbers (touches, clearances, score involvements), but it can be slower to pick up on a breakout season built more on pressure, tackling intensity and contested-ball grunt work that doesn't always show up as strongly in the raw stat line.

Encouragingly, the model's 3-vote game-by-game accuracy actually improved slightly on both 2023 and 2024, at `r round(confusion_matrix$byClass[4, 'Sensitivity']*100,2)`%. So while it stumbled on the season-long Medal winner, it's actually getting slightly better, game by game, at picking out who's having the best individual performance on the day - it's the accumulation of a whole season of near-misses at the margins, rather than a single glaring error, that cost it the winner this time.

Looking ahead, I'll carry the same modelling approach into 2026, with another season of data added to the training set. I'm also increasingly curious whether some measure of defensive pressure or ball-winning intensity - beyond what `Tackles` and `Contested.Possessions` already capture - might help close the gap on players like Rowell in future years.
