...
The model I developed this year is similar to my efforts in 2024, retaining the `Position` variable that was introduced to address the vote inflation seen in ruckmen and non-midfielders. As the Brownlow Medal is considered a midfielder award (for good reason), incorporating the playing position of players remains an important part of the model.
...
required_packages <- c(
  ...
  "ordinal",     #Package for fitting ordinal regression models
  "knitr",       #Package for dynamic report generation in R
  "unglue",      #Package for string manipulation
  "fastDummies", #Package for creating dummy variables
  "writexl",     #Package for exporting data to Excel files
  "DT",          #Package for creating interactive tables
  "caret",       #Package to help create a confusion matrix
  "ggplot2"      #Package for visual analysis and plotting
)
...
```{r}
#Read in the data
brownlow_data <- read.csv(
  if (file.exists("brownlow_data_2025.csv")) "brownlow_data_2025.csv"
  else "2025/brownlow_data_2025.csv"
)
```
...
```{r}
#Fallback for VS Code notebooks where DT::datatable may not render
datatable <- function(data, options = NULL, ...) {
...
Next, I created a training dataset by filtering the data for the specified seasons from 2015 to 2024.
...
So, according to the model, the top-ranked game by `Expected.Votes` highlights a standout individual performance from the 2025 season, followed by a number of other notable performances from across the competition.
