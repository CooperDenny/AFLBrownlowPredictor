...
Our model has done reasonably well. The headline result is that the model's top two predicted vote-getters matched the actual top two, in the exact same order: Patrick Cripps predicted first on 38 votes (he went on to smash the all-time record with 45), and Nick Daicos predicted second on 34 votes (he finished runner-up with 38). Getting the podium order right at the very top of the count, even if the exact tallies undersold just how dominant Cripps' season really was, is about as good a result as this kind of model can hope for.
...
In conclusion, adding the `Position` variable this year appears to have paid off. Compared to 2023, the model's Mean Absolute Error improved from 2.165 to 2.008, and rather than finishing third on the model's predicted leaderboard as Lachie Neale did last year, the actual 2024 Medallist landed exactly where the model said he would: first. Correcting for the ruckman bias I noticed in the 2023 results seems to have sharpened the model's read on genuine midfield dominance, rather than just rewarding whoever racks up the most raw touches.

The model's integration of Coaches' votes continues to be one of its most powerful features - players recognised by the coaches each week are consistently more likely to poll well with the umpires too, and the previous-season vote history variables help the model anchor onto genuinely consistent performers rather than one-off big games.

Where the model still comes up short is at the margins: 1 and 2 vote games remain genuinely hard to call, since a lot of best-on-ground performances are close-run things decided by very fine margins in the umpires' eyes. The model is clearly much more comfortable picking out the standout 3-vote performances than it is untangling who exactly finishes second and third on the night.

I'll apply this same approach again for 2025, carrying the `Position` variable forward and adding another season of data to the training set.
